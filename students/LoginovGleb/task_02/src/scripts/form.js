...
export function validateValue(value, fieldName) {
  ...
  if (
    ...
    return { valid: false, error: rules.messages.minLength };
  }
...
/**
 * Проверить валидность всех полей формы (чистая функция)
 * @param {{ name: string, email: string, message: string }} values - Значения полей
 * @returns {{ isValid: boolean, errors: { name: string | null, email: string | null, message: string | null } }}
 */
export function validateFormValues(values) {
  const nameResult = validateValue(values.name || "", "name");
  const emailResult = validateValue(values.email || "", "email");
  const messageResult = validateValue(values.message || "", "message");

  return {
    isValid: nameResult.valid && emailResult.valid && messageResult.valid,
    errors: {
...
function isFormValid(form) {
  ...
  const messageValid = messageInput
    ? isFieldValid(messageInput, "message")
    : false;
